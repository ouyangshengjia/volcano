...
func TestNewPodBunchInfo(t *testing.T) {
	type args struct {
		uid         BunchID
		job         JobID
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := NewPodBunchInfo(tt.args.uid, tt.args.job, tt.args.policy, tt.args.matchValues)
			assert.Equalf(t, tt.want, got, "NewPodBunchInfo(%v, %v, %v, %v)", tt.args.uid, tt.args.job, tt.args.policy, tt.args.matchValues)
		})
	}
}

func TestPodBunchInfo_IsHardTopologyMode(t *testing.T) {
	type fields struct {
		UID               BunchID
		Job               JobID
		Priority          int32
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotIsHard, gotTier := pbi.IsHardTopologyMode()
...
func TestPodBunchInfo_IsSoftTopologyMode(t *testing.T) {
	type fields struct {
		UID               BunchID
		Job               JobID
		Priority          int32
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equalf(t, tt.expectedHard, pbi.IsSoftTopologyMode(), "IsSoftTopologyMode()")
		})
	}
}

func TestPodBunchInfo_getPodBunchMatchValues(t *testing.T) {
	type args struct {
		policy scheduling.BunchPolicySpec
		pod    *v1.Pod
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equalf(t, tt.want, getPodBunchMatchValues(tt.args.policy, tt.args.pod), "getPodBunchMatchValues(%v, %v)", tt.args.policy, tt.args.pod)
		})
	}
}

func TestPodBunchInfo_getPodBunchId(t *testing.T) {
	type args struct {
		job         JobID
		policy      string
		matchValues []string
	...
	tests := []struct {
	...
	}{
		{
			...
			want: BunchID("job1/policy1-value1-value2"),
		},
		{
			...
			want: BunchID("job1/policy1-value1"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := getPodBunchId(tt.args.job, tt.args.policy, tt.args.matchValues)
			if got != tt.want {
				t.Errorf("getPodBunchId(%v, %v, %v) got = %v, want %v", tt.args.job, tt.args.policy, tt.args.matchValues, got, tt.want)
			}
		})
	}
}

func TestPodBunchInfo_IsPipelined(t *testing.T) {
	tests := []struct {
		name     string
		pbi      *PodBunchInfo
		expected bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.pbi.IsPipelined()
			assert.Equal(t, tt.expected, result)
		})
	}
}

func TestPodBunchInfo_IsReady(t *testing.T) {
	tests := []struct {
		name     string
		pbi      *PodBunchInfo
		expected bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.pbi.IsReady()
			assert.Equal(t, tt.expected, result)
		})
	}
}

func TestPodBunchInfo_getTaskHighestPriority(t *testing.T) {
	// Test case 1: When taskPriorities is empty, it should return 0
	pbi := &PodBunchInfo{
		taskPriorities: map[int32]sets.Set[TaskID]{},
	}
	assert.Equal(t, int32(0), pbi.getTaskHighestPriority(), "Expected 0 when taskPriorities is empty")
	...
	assert.Equal(t, int32(1), pbi.getTaskHighestPriority(), "Expected 1 when taskPriorities has one element")
	...
	assert.Equal(t, int32(3), pbi.getTaskHighestPriority(), "Expected 3 when taskPriorities has multiple elements")
	...
	assert.Equal(t, int32(0), pbi.getTaskHighestPriority(), "Expected -1 when taskPriorities has multiple negative elements")
}
