...
### Option 1 – Dynamically Adjust Offline Workload CPU Quota Based on Node CPU Utilization
...
### Option 2 – Leverage Kubernetes Native Pod QoS Classes
...
- When oversubscription is enabled, the system periodically calculates the available oversubscribed CPU resources and publishes the result as a internal **Event**.
...
- To suspress gently, each adjustment changes `cpu.max` by no more than `adjustStep * node.allocatable[cpu]`.
